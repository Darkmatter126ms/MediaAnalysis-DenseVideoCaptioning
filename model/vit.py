...
class VisionTransformer(nn.Module):
    """ Vision Transformer
    """
    def __init__(self, num_features=100, video_embedding_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_features, video_embedding_dim))
        self.pos_drop = nn.Dropout(p=drop_rate)

        ## Attention Blocks
        self.blocks = nn.ModuleList([
            Block(
                dim=video_embedding_dim, num_heads=num_heads, mlp_dim=mlp_dim, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, norm_layer=norm_layer)
            for _ in range(self.depth)])
        self.norm = norm_layer(video_embedding_dim)
